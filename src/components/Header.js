...
import { UserContext } from "../components/UserContext"; // We'll create this context
...
const Header = () => {
  ...
  return (
    <header className="flex items-center justify-between w-full px-4 py-2 bg-white border-b border-gray-200 relative">
        ...
        <nav className="flex items-center space-x-6">
          ...
          <div className="relative">
            <div
              ...
              className={`${
                isActive("/forklift") || isActive("/sorter") || isActive("/agv")
                  ? "text-green-500 font-bold"
                  : "text-gray-800 font-medium"
            ...
            {isDropdownOpen && (
              <div className="absolute left-0 mt-2 bg-white border rounded shadow-md min-w-[80px] z-10">
                <Link to="/agv" onClick={closeDropdown}>
                  <div className="px-4 py-2 text-gray-800 hover:bg-green-100">
                    AGV
                  </div>
                </Link>
      ...
      {/* 외부 클릭 감지용 배경 */}
      {isDropdownOpen && (
        <div onClick={closeDropdown} className="fixed inset-0 z-0"></div>
      )}
    </header>
  );
};
